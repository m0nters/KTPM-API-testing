pipeline {
    ...
    environment {
        ...
        // Secrets - these should be configured in Jenkins credentials
        SECRET_DB = credentials('SECRET_DB')
        SECRET_DB_USER = credentials('SECRET_DB_USER')
        SECRET_DB_PASS = credentials('SECRET_DB_PASS')
        SECRET_GOOGLE_ID = credentials('SECRET_GOOGLE_ID')
        SECRET_GOOGLE_SECRET = credentials('SECRET_GOOGLE_SECRET')
        SECRET_GITHUB_ID = credentials('SECRET_GITHUB_ID')
        SECRET_GITHUB_SECRET = credentials('SECRET_GITHUB_SECRET')
    }
                ...
                stage('Laravel Unit Tests') {
                    steps {
                        script {
                            ...
                            // Install PHP if not available (this depends on your Jenkins setup)
                ...
                stage('UI Tests Setup') {
                    steps {
                        script {
                            ...
                            // Use NodeJS plugin or install Node.js
        ...
        stage('Deploy') {
            ...
            steps {
                script {
                    ...
                    echo "🐘 Setting up PHP..."
                    // Setup PHP (depends on your Jenkins configuration)
                    
                    echo "⚙️ Setting up Node.js..."
                    // Setup Node.js (depends on your Jenkins configuration)
